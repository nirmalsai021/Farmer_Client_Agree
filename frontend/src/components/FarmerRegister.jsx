...
const FarmerRegister = ({ setStoredFarmer }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const exactMatchUser = registeredFarmers.find(
      (farmer) =>
        farmer.username.toLowerCase() === formData.username.trim().toLowerCase() &&
        farmer.email.toLowerCase() === formData.email.trim().toLowerCase()
    );
    if (exactMatchUser) {
    ...
    const usernameTaken = registeredFarmers.find(
      (farmer) =>
        farmer.username.toLowerCase() === formData.username.trim().toLowerCase() &&
        farmer.email.toLowerCase() !== formData.email.trim().toLowerCase()
    );
    ...
    const emailTaken = registeredFarmers.find(
      (farmer) =>
        farmer.email.toLowerCase() === formData.email.trim().toLowerCase() &&
        farmer.username.toLowerCase() !== formData.username.trim().toLowerCase()
    );
    ...
    localStorage.setItem("registeredFarmers", JSON.stringify(registeredFarmers));
    ...
    localStorage.setItem("farmer", JSON.stringify(farmerInfo));
    if (setStoredFarmer) setStoredFarmer(farmerInfo);

    alert(`Farmer registration successful for ${newFarmer.username}`);
  ...
  return (
    <div className="farmer-register">
      ...
      <form onSubmit={handleSubmit} className="farmer-register-form" noValidate>
